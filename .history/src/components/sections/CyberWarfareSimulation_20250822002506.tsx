...
const CyberWarfareSimulation = () => {
  ...
  const startAttackSequence = async () => {
    ...
    const firewallPath = createAttackPath('external', 'firewall');
  ...
  const getNodeColor = (status: NetworkNode['status'], health: number) => {
    switch (status) {
                ...
                {nodes.map(node => (
                  <g key={node.id}>
                    <motion.circle
                      ...
                      cy={`${node.y}%`}
                      r="8"
                      fill={getNodeColor(node.status, node.health)}
                      stroke="#fff"
                      strokeWidth="2"
                      animate={{
