...
export default function FloatingActionMenu() {
  ...
  // Get tooltip position based on item position
  const getTooltipPosition = (index: number, total: number) => {
    const baseAngle = -90;
    const angleStep = 360 / total;
    const angle = baseAngle + (index * angleStep);
    
    // Determine tooltip direction based on angle
    if (angle >= -45 && angle <= 45) return 'right'; // Top area
    if (angle > 45 && angle <= 135) return 'left';   // Right area  
    if (angle > 135 && angle <= 225) return 'right'; // Bottom area
    return 'left'; // Left area
  };
  ...
  const handleDragStart = useCallback(() => {
    ...
    // Tắt animation để tăng performance
    if (hideTimeoutRef.current) {
      clearTimeout(hideTimeoutRef.current);
    }
  }, []);
  ...
  return (
    <motion.div
      data-fab-container
      className="fixed bottom-6 left-6 z-[55] w-16 h-16" // Increased z-index
      drag
      dragMomentum={false}
      dragElastic={0.05}
      dragTransition={{
        ...
        <AnimatePresence mode="wait">
          {isOpen && menuItems.map((item, index) => (
            ...
            >
              ...
              {/* Tooltip - smart positioning based on item location */}
              {(() => {
                const tooltipDir = getTooltipPosition(index, menuItems.length);
                const isLeft = tooltipDir === 'left';
                return (
                  <div className={`absolute ${isLeft ? 'right-full mr-3' : 'left-full ml-3'} top-1/2 transform -translate-y-1/2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap pointer-events-none z-[60] shadow-xl border border-gray-700`}>
                    {item.label}
                    {/* Arrow pointer */}
                    <div className={`absolute ${isLeft ? 'left-full' : 'right-full'} top-1/2 transform -translate-y-1/2 border-4 border-transparent ${isLeft ? 'border-l-gray-900' : 'border-r-gray-900'}`}></div>
                  </div>
                );
              })()}
            </motion.button>
