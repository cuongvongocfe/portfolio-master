...
export default function FloatingActionMenu() {
  ...
  const handleDragEnd = () => {
    ...
    setTimeout(() => {
      const button = document.querySelector('[data-fab-button]') as HTMLElement;
      if (!button) return;
      ...
      // Keep current vertical position but constrain
      newY = Math.max(24, Math.min(viewportHeight - buttonSize - 24, rect.top));
      
      // Smooth animate to new position
      button.style.transform = `translate(${newX - 24}px, ${newY - (viewportHeight - buttonSize - 24)}px)`;
  ...
  return (
    <>
      {/* Drag Constraints Container - toàn màn hình nhưng với padding */}
      <div 
        ref={constraintsRef} 
